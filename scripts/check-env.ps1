...
$jarPath    = Join-Path $root 'backend-java\target\medication-radar-backend-2.0.0-SNAPSHOT.jar'
...
$rows.Add([pscustomobject]@{ 项目 = 'Redis 服务(6379)'; 状态 = if (Test-Port 6379) { 'OK' } else { '未运行' }; 详情 = '可选，未装自动降级；可运行 scripts\setup-redis.ps1 安装便携版' })
...
Write-Host ''
Write-Host '================ 用药雷达 环境体检 ================' -ForegroundColor Cyan
$rows | Format-Table 项目, 状态, 详情 -AutoSize
Write-Host '说明：Java / Node 缺失时无需手动安装，直接双击「用药雷达.bat」，'
