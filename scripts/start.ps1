...
<#
用药雷达一键启动脚本（便携版：适配 GitHub/Gitee 下载即用）

流程：环境自检（缺失可自动下载便携运行时到项目 runtime\）→ 首次运行向导生成 .env
      → 检测 MySQL → 首次自动安装前端依赖 → 启动后端/前端 → 就绪后打开浏览器

用法：
    双击项目根目录「用药雷达.bat」
    或在项目根目录执行：powershell -ExecutionPolicy Bypass -File .\scripts\start.ps1

停止：
    关闭本脚本窗口（会连同后端/前端一起停止）
    或双击项目根目录「停止-用药雷达.bat」
#>
...
$jarName     = 'medication-radar-backend-2.0.0-SNAPSHOT.jar'
...
function Ensure-DotEnv {
    ...
    Write-Hint 'DeepSeek API Key 不在命令行询问：启动后由管理员在网页「知识库管理 → AI 服务设置」中填写并加密保存'
    return $true
}
...
function Ensure-DesktopShortcut {
    try {
        ...
        $bat = Join-Path $root '用药雷达.bat'
        $ico = Join-Path $root 'assets\medication-radar.ico'
        $s = $ws.CreateShortcut((Join-Path $desktop '用药雷达.lnk'))
        $s.TargetPath = $bat
        $s.WorkingDirectory = $root
        $s.WindowStyle = 1
        if (Test-Path $ico) { $s.IconLocation = $ico }
        $s.Description = '用药雷达一键启动'
        $s.Save()
        Write-Ok '桌面快捷方式已就绪（用药雷达）'
    } catch {
...
Write-Host '  用药雷达 · 便携启动脚本' -ForegroundColor Cyan
Write-Host '================================' -ForegroundColor Cyan
...
foreach ($p in @(8080, 5173)) {
    if (Test-Port $p) {
        ...
        Write-Host '  双击项目根目录「停止-用药雷达.bat」停止已有服务后重试' -ForegroundColor Red
        Pause-Exit
    }
}
...
if (-not (Test-Port 3306)) {
    ...
    Write-Host '  本项目需要一个 MySQL 8.0 服务；Redis 为可选（未装自动降级）。' -ForegroundColor Red
...
# ---------- 3.5 Redis（可选：用于限流/令牌注销；未装自动降级，是否安装由用户选择） ----------
...
if (Test-Port 6379) {
    ...
    if (Test-Path $redisExe) {
        ...
        if ($ans -eq '' -or $ans -match '^[Yy]') {
            ...
            Write-Hint '已跳过 Redis：限流/令牌注销自动降级，其他功能不受影响'
        }
    } else {
        Write-Hint 'Redis(6379) 未运行：可选组件，不装也能用（限流放行/注销跳过，自动降级）'
...
if (-not (Test-Path $jarPath)) {
    ...
    Write-Host '    2. 安装 JDK 17+ 与 Maven 后，在 backend-java 目录执行：mvn -DskipTests package'
    Pause-Exit
}
Write-Ok '后端 jar 已就绪'
...
$frontendProc = Start-Process -FilePath $node.Npm -ArgumentList 'run', 'dev', '--', '--host' `
...
if ($dbIssue) {
    ...
    Write-Host '  最常见原因：'
    Write-Host '    1. 尚未执行建库 SQL（medication_radar 库或表不存在）'
...
Write-Host ' 用药雷达已启动！' -ForegroundColor Green
Write-Host '  前端入口   http://localhost:5173' -ForegroundColor Green
Write-Host '  后端接口   http://localhost:8080' -ForegroundColor Green
Write-Host ' 停止服务：关闭本窗口，或双击「停止-用药雷达.bat」' -ForegroundColor Green
Write-Host '=====================================================' -ForegroundColor Green
Write-Host ''
