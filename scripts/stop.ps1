...
$jarMarker = 'medication-radar-backend'

Write-Host ''
Write-Host '==> 正在停止用药雷达（后端 8080 / 前端 5173）...' -ForegroundColor Cyan
...
# 兜底：清掉命令行里挂着本项目 jar 的残留 java 进程
...
Write-Host ''
if ($found) {
    Write-Host '用药雷达已全部停止。' -ForegroundColor Green
} else {
    Write-Host '没有发现正在运行的用药雷达服务。' -ForegroundColor Green
}
